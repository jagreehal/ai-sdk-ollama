/**
 * streamTextOllama - Enhanced streamText with Ollama-specific reliability
 *
 ...
 */
export async function streamTextOllama(options: StreamTextOllamaOptions) {
  ...
  if (!hasTools || !enableStreamingSynthesis) {
    return streamText(streamTextOptions as Parameters<typeof streamText>[0]);
  }

  // Enhanced streaming with tool calling reliability
  const streamResult = await streamText(
    streamTextOptions as Parameters<typeof streamText>[0],
  );
...
Based on the tool results above, please provide a comprehensive response to the original question.`;
      ...
      try {
        ...
        const synthesisStream = await streamText({
          ...baseOptions,
