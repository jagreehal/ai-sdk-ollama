 ...
 *
 * Perfect for social media videos showing why ai-sdk-ollama is essential
 *
 * THE PROBLEM: Standard Ollama providers execute tools but return EMPTY responses
 * THE SOLUTION: ai-sdk-ollama guarantees complete, useful responses
 *
 * Ready to record? Run: npx tsx generateText-vs-generateTextOllama.ts
 */

import { ollama, generateTextOllama } from 'ai-sdk-ollama';
...
// Perfect demo scenarios for social media video
const testScenarios = [
  {
...
function logResult(provider: string, success: boolean, details: TestResult) {
  ...
  console.log(''); // Add spacing for video
}
...
async function testGenerateTextOllama(scenario: typeof testScenarios[number]): Promise<TestResult> {
  try {
    console.log(`\n  🚀 Testing with generateTextOllama (enhanced)...`);
...
async function runComparison() {
  ...
  console.log(styleText('dim', 'Comparing standard generateText vs generateTextOllama'));
  ...
  for (const scenario of testScenarios) {
    ...
    console.log(styleText(['bold', 'green'], '🚀 Enhanced generateTextOllama:'));
    const enhancedResult = await testGenerateTextOllama(scenario);
    logResult('generateTextOllama', enhancedResult.success, enhancedResult);

    if (enhancedResult.success) enhancedSuccessCount++;

    // Dramatic pause for video effect
  ...
  console.log(`${styleText(['bold', 'green'], '✅ generateTextOllama:')} ${enhancedSuccessCount}/${testScenarios.length} scenarios worked`);

  const successRate = Math.round((enhancedSuccessCount / testScenarios.length) * 100);
  console.log(`\n${styleText(['bold', 'magenta'], '🎯 Success rate:')} ${successRate}% with generateTextOllama`);

  console.log('\n' + styleText(['bold', 'cyan'], '💡 Key takeaways:'));
  console.log('• Standard functions often return empty responses after tool execution');
  console.log('• generateTextOllama provides reliable, complete responses');
  ...
  console.log('Just change generateText → generateTextOllama');
