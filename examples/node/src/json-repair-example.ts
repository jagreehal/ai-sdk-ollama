  ...
  try {
    ...
    const result1 = await generateText({
      ...
      output: Output.object({
        schema: z.object({
          name: z.string(),
          email: z.string(), // Simple string validation (Zod email() is too strict for LLM output)
          age: z.number(),
          city: z.string(),
        }),
      }),
      prompt: 'Generate a person profile for John Doe, 30 years old, from New York. Use a simple email format.',
    });
    ...
    try {
      ...
      console.log('✅ Succeeded without reliability:', JSON.stringify(result3.output, null, 2));
    } catch (error) {
    ...
    try {
      ...
      console.log('✅ Succeeded with retries but no repair:', JSON.stringify(result4.output, null, 2));
    } catch (error) {
