 ...
 *
 * Perfect for social media videos showing streaming reliability
 *
 * THE PROBLEM: Standard streamText executes tools but streams no final text
 * THE SOLUTION: streamTextOllama provides reliable streaming responses
 *
 * Ready to record? Run: npx tsx streamText-vs-streamTextOllama.ts
 */

import { ollama, streamTextOllama } from 'ai-sdk-ollama';
...
async function testStreamTextOllama(scenario: typeof streamScenarios[number]): Promise<StreamResult> {
  try {
    console.log(`\n  🚀 Testing with streamTextOllama (enhanced)...`);
...
async function runStreamingComparison() {
  ...
  console.log(styleText('dim', 'Comparing standard streamText vs streamTextOllama'));
  ...
  for (const scenario of streamScenarios) {
    ...
    console.log(styleText(['bold', 'green'], '🚀 Enhanced streamTextOllama:'));
    const enhancedResult = await testStreamTextOllama(scenario);
    logStreamResult('streamTextOllama', enhancedResult.success, enhancedResult);

    if (enhancedResult.success) enhancedSuccessCount++;

    // Pause for dramatic effect in video
  ...
  console.log(`${styleText(['bold', 'green'], '✅ streamTextOllama:')} ${enhancedSuccessCount}/${streamScenarios.length} scenarios worked`);

  const successRate = Math.round((enhancedSuccessCount / streamScenarios.length) * 100);
  console.log(`\n${styleText(['bold', 'magenta'], '🎯 Success rate:')} ${successRate}% with streamTextOllama`);

  console.log('\n' + styleText(['bold', 'cyan'], '💡 Key takeaways:'));
  console.log('• Standard streaming often executes tools but streams no final text');
  console.log('• streamTextOllama provides reliable, real-time streaming responses');
  ...
  console.log('Just change streamText → streamTextOllama');
