...
async function demonstrateModelCapabilities() {
  ...
  for (const modelId of testModels) {
    ...
    console.log(
      `Tool calling: ${modelInfo.capabilities.supportsToolCalling ? '✅' : '❌'}`,
    );
    console.log(
      `Vision: ${modelInfo.capabilities.supportsVision ? '✅' : '❌'}`,
    );
    console.log(
      `JSON mode: ${modelInfo.capabilities.supportsJsonMode ? '✅' : '❌'}`,
    );
    console.log(
      `Context window: ${modelInfo.capabilities.contextWindow.toLocaleString()} tokens`,
    );
  }
  ...
  const toolCallingSuggestions = suggestModelsForFeatures({
    toolCalling: true,
  });
